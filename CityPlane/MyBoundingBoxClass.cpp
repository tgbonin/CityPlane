...
bool MyBoundingBoxClass::SAT(MyBoundingBoxClass* const a_pOther)
{
	...
	if (std::abs(v3Distance.x) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		m_pMeshMnger->AddPlaneToQueue(m4Space * glm::scale(vector3(5.0f)), RERED);
#endif
		return false;
	}
	...
	if (std::abs(v3Distance.y) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		m_pMeshMnger->AddPlaneToQueue(m4Space * glm::scale(vector3(5.0f)), REGREEN);
#endif
		return false;
	}
	...
	if (std::abs(v3Distance.z) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		m_pMeshMnger->AddPlaneToQueue(m4Space * glm::scale(vector3(5.0f)), REBLUE);
#endif
		return false;
	}
	...
	if (std::abs(v3Distance.x * m3Rotation[0][0] + v3Distance.y * m3Rotation[1][0] + v3Distance.z * m3Rotation[2][0]) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		m_pMeshMnger->AddPlaneToQueue(m4Space * glm::scale(vector3(5.0f)), RERED * 0.33f);
#endif
		return false;
	}
	...
	if (std::abs(v3Distance.x * m3Rotation[0][1] + v3Distance.y * m3Rotation[1][1] + v3Distance.z * m3Rotation[2][1]) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		m_pMeshMnger->AddPlaneToQueue(m4Space * glm::scale(vector3(5.0f)), REGREEN * 0.33f);
#endif
		return false;
	}
	...
	if (std::abs(v3Distance.x * m3Rotation[0][2] + v3Distance.y * m3Rotation[1][2] + v3Distance.z * m3Rotation[2][2]) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		m_pMeshMnger->AddPlaneToQueue(m4Space * glm::scale(vector3(5.0f)), REBLUE * 0.33f);
#endif
		return false;
	}
	...
	if (std::abs(v3Distance.z * m3Rotation[1][0] - v3Distance.y * m3Rotation[2][0]) > fCenterAToMiddle + fCenterBToMiddle)
	{
		...
		v3Center /= 2.0f;
		matrix4 m4Space = glm::translate(m_m4ToWorld, v3Center);
		m_pMeshMnger->AddSphereToQueue(m4Space * glm::scale(vector3(0.10f)), REYELLOW, SOLID);
#endif
		return false;
	}
...
bool MyBoundingBoxClass::IsColliding(MyBoundingBoxClass* const a_pOther)
{
	...
	*/
	...
	if ((m_fRadius + a_pOther->m_fRadius) < glm::distance(m_v3CenterG, a_pOther->m_v3CenterG))
		return false;
	//If we get a positive we might have a collision, check the SAT, you can also try the ARBB as a pretest
	//but I will skip it for this example
