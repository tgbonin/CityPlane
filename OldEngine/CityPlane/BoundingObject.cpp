...
BoundingObject::~BoundingObject(){ Release(); };
//Accessors

void BoundingObject::SetVisibility(bool a_bVisibility)
{
	m_bIsVisible = a_bVisibility;
}

void BoundingObject::SetColour(vector3 a_v3Colour){
	m_v3Colour = a_v3Colour;
}
...
float BoundingObject::GetRadiusG(void){ return m_fRadius; }
...
bool BoundingObject::IsColliding(BoundingObject* const a_pOther)
{
	...
	*/
	...
	//Sphere Collision Pre-Test
	float distance = glm::distance(m_v3CenterG, a_pOther->GetCenterGlobal());
	if (distance < m_fRadius + a_pOther->GetRadiusG())
	{
		//Check for X
...
void BoundingObject::DrawBoundingObjects(MeshManagerSingleton* a_pMeshMngr)
{
	if (m_bIsVisible) {
		a_pMeshMngr->AddCubeToQueue(glm::translate(m_v3CenterG) * glm::scale(m_v3HalfWidthG * 2.0f), m_v3Colour, WIRE);
		a_pMeshMngr->AddCubeToQueue(m_m4ToWorld * glm::translate(matrix4(1.0f), m_v3Center) * glm::scale(m_v3HalfWidth * 2.0f), m_v3Colour, WIRE);
		//a_pMeshMngr->AddSphereToQueue(glm::translate(IDENTITY_M4, m_v3CenterG) * glm::scale(vector3(m_fRadius) * 2.0f), m_v3Colour, WIRE);
	}	
}
